...
{$ENDIF}
  ...
  Tposrad = record
    ...
    radd, raddn:single;
{$IFDEF terraineditor}
    individual:boolean; //azaz terrain_modifierbol sz�rmazik-e
    name:string;
{$ENDIF}
    //ground:boolean;
  end;
...
begin
  ...
  begin
    ...
    if (hmnev <> '') then
      if LTFF(a_d3dDevice, 'data/textures/' + hmnev, heightmap, TEXFLAG_COLOR) then //text�ra n�lk�l �gysem �r semmit a PO
...
{$R-}
  ...
  begin
    ...
    begin // ha not vanLM akkor null�z�dik
  ...
  begin
    ...
    begin
      begin
        ...
        D3DXVec3Subtract(pl, triangles[miket[i]].v1, triangles[miket[i]].v2); //b�rmely 2 pont a s�kban
...
{$IFDEF magic}
  ...
  //          lu2:=lerp(triangles[miket[i]].lu0,triangles[miket[i]].lu1,tavPointPoint(triangles[miket[i]].v0,v2)/tavpointpoint(triangles[miket[i]].v0,triangles[miket[i]].v1));
            ...
            if not D3DXIntersectTri(v0, v1, v2,
              ...
              baryU, baryV,
              pdist) then
              Writeln(logfile, 'Internal error: mitlo'); //TODO optimaliz�lt fv.
...
var
  ...
  // lmapsize:integer; //cell�kban (64x64)        NOPE
  cursize:integer;
  tryx, tryy:integer;
  bol:boolean;
  Xbitmap:Tbitmap;
  ...
  for i:=0 to high(ojjektumarr) do
    for j:=0 to ojjektumarr[i].hvszam - 1 do
      ...
      begin
        ...
        if ttav > sqr(max(150, ojjektumarr[i].rad2 * 12)) then // a 12 az imposzer bekapcs t�vols�g
        begin
          kelluj:= not imposter;
    ...
    for j:=0 to drawtable[i].xasz - 1 do
      if drawtable[i].DIPdata[j * ojjektumarr[i].hvszam].tex = k then
        with drawtable[i].DIPdata[j * ojjektumarr[i].hvszam + jj] do
          if facecount > 0 then
          begin
            g_pd3ddevice.SetTexture(1, ojjektumarr[i].lightmap); //a szop�s ir�nya: le
