...
type
  ...
  TGridElem=packed record
    meret,top:integer;
    elemek:Pointer;//1D Array, Dwordok term�szetesen Castolhat� pointerre de �hgyis index lesz
  end;
...
var
  ...
  hudMessages:array[0..4] of THUDmessage;//�ll�that� mindk�t v�ge, az alacsony a friss
  ...
  texture_res:byte;// 0 low, 1 med, 2 hi, //�j: 100 superlow, 101 low, 102, med, 103 high, 104 maximum
  fakedeath:single;
  vallmag:single=1.2;
  vallmag2:single=1.5;
...
const
  ...
  POSTPROC_MIN=0;
  POSTPROC_DISTORTION=1;//�gy haszn�ld, hogy kereshet� legyen
  POSTPROC_LASTNONSHADER=POSTPROC_DISTORTION;
  POSTPROC_SNIPER=2;
  POSTPROC_GLOW=3;
  POSTPROC_MAX=3;

  //mikor van csicsa hdr: ha van gpeffect �s detail > min
  ...
  FEGYV_BM3_2=101;
  FEGYV_BM3_3=102;// k�l�nb�zo goly�fajt�k // Hector ne l�gy balfasz.
...
begin
  for i:=low(muted)to high(muted) do
  begin
    if name=muted[i] then muted[i]:='';//ment�skor sz�rni kell
  end;
end;
...
begin
  case fegyv of
    ...
    FEGYV_HPL:result:=1;
  else result:=0;//ezekn�l �gysincs muzz
  end;
end;
...
begin
  ...
  result:=(i mod 62)+1;//kibaszott string 1..62 �s nem 0..61
end;
...
begin
  NoNANINF(mi);
  //  if (mi<-5000) and (mi>5000) then //�lt: 0.1-2.9.2.1
...
//function fegyindex(fegy:byte):byte;//ideiglenes, jsonb�l kell
//
//
//begin
//  case fegy of
