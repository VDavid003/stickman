...
uses
sysutils,math,typestuff,D3DX9,ojjektumok,windows;
const
WPsuruseg=1;
type
 TWaypoint = record
...
begin
 ...
 begin
  ...
  for k:=0 to high(ojjektumok) do
   for j:=0 to ojjektumok[k].hvszam-1 do
    if ojjektumok[k].raytestbol(v1,v2,j,COLLISION_SOLID) then goto tbc;
 ...
 begin
  ...
  for k:=0 to high(ojjektumok) do
   for j:=0 to ojjektumok[k].hvszam-1 do
    if ojjektumok[k].raytestbol(v1,v2,j,COLLISION_SOLID) then goto vege;
